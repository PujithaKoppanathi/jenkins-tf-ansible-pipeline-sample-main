locals {
  vmss_tags = merge(
    {
      "load_balancer_name"   = "shd--${var.lb_name}"
      "fds:build_access_set" = var.build_access_set
      "fds:HostRole"         = var.lb_name
      ...
      "fds:cloud_resource"   = "shared load balancer"
    },
    var.vmss_tags
  )
  stack_tag = "SHD-LB---build--${var.image_name_trimmed}---ws--${var.param_workspace_name_trimmed}"
  ...
  sg_ingress = concat(
    flatten([
      for target_group in var.target_groups : [{
        ...
        protocol    = target_group.backend_protocol
    ...
    flatten([
      for ingress in var.user_defined_sg_ingress : [{
        ...
        self        = ingress.self ? true : false
        cidr_blocks = ingress.self ? [] : ["10.0.0.0/8", "164.55.0.0/16", "172.16.0.0/12"]
      }]
    ])
