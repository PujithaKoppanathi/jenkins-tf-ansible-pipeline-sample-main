locals {
  ...
  /*sg_ingress = concat(
    flatten([
      for load_balancer in var.load_balancers : [{
        from_port   = load_balancer.backend_port
  ...
  /*sg_ingress = concat(
    flatten([
      for load_balancer in var.load_balancers[*] : [{
        from_port   = (load_balancer.target_groups) => load_balancer.target_groups
        to_port     = load_balancer.target_groups
        protocol    = load_balancer.target_groups
        cidr_blocks = ["10.0.0.0/8", "164.55.0.0/16", "172.16.0.0/12"]
      }]
    ]),
    //var.user_defined_sg_ingress
  ...
  sg_ingress = concat(flatten([
    for lb_key, load_balancer in var.load_balancers : [
