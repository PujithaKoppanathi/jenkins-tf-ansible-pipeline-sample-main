locals {
  vmss_tags = merge(
    {
      "load_balancer_pgs"    = jsonencode(var.pg_prefix)
      ...
      "fds:cloud_resource"   = "auto scaling group with shared load balancer"
    },
    var.vmss_tags
  )
  stack_tag = "ASG-SHD-LB---build--${var.image_name_trimmed}---ws--${var.param_workspace_name_trimmed}"
